<?php
    ...
    $sqlLote = "SELECT * FROM reagentes_lotes WHERE QTDE_LOTE > 0 ORDER BY DATA_VALI ASC";
    $listaLote = $conectDB->query($sqlLote) or die();
?>
<!DOCTYPE html>
<html lang="pt-br">
<head>
...
<body>
    <div class="mainBox">
        ...
        <br><br>
        <p style="text-align:center; font-size:16px; ">Lista de Lotes de Reagentes</p>
        <table >
            <thead>
                <tr>
                    <th style="width: 100px; font-size: 12px; ">Código do Reagente</th>
                    <th style="width: 590px; font-size: 12px; ">Descrição do Reagente</th>
                    <th style="width: 100px; font-size: 12px; ">Numero do Lote</th>
                    <th style="width: 120px; font-size: 12px; ">Quantidade em Estoque</th>
                    <th style="width: 100px; font-size: 12px; ">Data de Validade</th>
                </tr>
            </thead>
            <tbody>
                <?php while($dado2 = $listaLote->fetch_array()) { ?> 
                <tr>
                    <td style="font-size: 12px; width: 100px; height: 30px; text-align:center;"> <?php echo $dado2['CODIGO_REAGENTE'];?> </td> 
                    <td style="font-size: 12px; width: 590px; height: 30px;"                   > <?php echo $dado2['DESCR_REAGENTE']; ?> </td>
                    <td style="font-size: 12px; width: 100px; height: 30px; text-align:center;"> <?php echo $dado2['NUMERO_LOTE'];?> </td>
                    <td style="font-size: 12px; width: 120px; height: 30px; text-align:center;"> <?php echo $dado2['QTDE_LOTE'];?> </td>
                    <td style="font-size: 12px; width: 100px; height: 30px; text-align:center;"> <?php echo date('d/m/Y', strtotime($dado2['DATA_VALI']));?> </td>
                </tr> 
                <?php } ?>
            </tbody>
        </table>
