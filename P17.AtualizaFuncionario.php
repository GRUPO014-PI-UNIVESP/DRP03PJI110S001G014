<?php
    ...
    if(!isset($_SESSION['usuario'])){ 
     ...
     if(!empty($_GET['id'])){
        ...
        if($verificador == 0){
            header('Location: P14.AtualizaFuncionario.php');
        }
     ...
     if(isset($_POST['submit'])){
        $idFc =            $_SESSION['nuIDFunc'];
        $tele =            $_POST['telefone'];
        $emai = strtolower($_POST['email']);
        $admi =            $_POST['admissao'];
        $dept = strtoupper($_POST['departamento']);
        $carg = strtoupper($_POST['cargo']);
        $nCep =            $_POST['cep'];
        $sUFs = strtoupper($_POST['estado']);
        $city = strtoupper($_POST['cidade']);
        $bair = strtoupper($_POST['bairro']);
        $logr = strtoupper($_POST['rua']);
        $nRes =            $_POST['nResid'];
        switch($carg){
        ...
        $atualiza = mysqli_query($conectDB, "UPDATE funcionario SET TELEFONE_FUNCIONARIO = '$tele', EMAIL_FUNCIONARIO = '$emai', 
                                    DEPARTAMENTO = '$dept', CARGO = '$carg', CREDENCIAL = '$cred', DOMICILIO_CEP = '$nCep',
                                    DOMICILIO_RUA = '$logr', DOMICILIO_NUMERO = '$nRes', DOMICILIO_BAIRRO = '$bair',
                                    DOMICILIO_CIDADE = '$city', DOMICILIO_UF = '$sUFs' WHERE ID_FUNCIONARIO = '$idFc'");
...
<script>
	...
	$("#cnpj").mask("99.999.999/9999-99");
        ...
        <div class="mainBox">
            ...
            <fieldset>
                <legend>Quadro de Funcionários - Edição</legend>
                <br>
                <div class="box1esq">
                    <div class="inputBox">
                ...
                <div class="box6mid">
                    <input type="radio" id="departamento" name="departamento" style="text-transform: uppercase" value="ADMINISTRAÇÃO">        <label for="">Administração</label><br>
                    <input type="radio" id="departamento" name="departamento" style="text-transform: uppercase" value="GARANTIA DE QUALIDADE"><label for="">Garantia de Qualidade</label><br>
                    <input type="radio" id="departamento" name="departamento" style="text-transform: uppercase" value="LOGÍSTICA">            <label for="">Logística</label><br>
                    <input type="radio" id="departamento" name="departamento" style="text-transform: uppercase" value="PRODUÇÃO">             <label for="">Produção</label><br>
                </div>
                <div class="box6dir">
                    <input type="radio" id="cargo" name="cargo" style="text-transform: uppercase" value="DIRETOR(A)">   <label for="cargo">Diretor(a)</label><br>
                    <input type="radio" id="cargo" name="cargo" style="text-transform: uppercase" value="SUPERVISOR(A)"><label for="cargo">Supervisor(a)</label><br>
                    <input type="radio" id="cargo" name="cargo" style="text-transform: uppercase" value="ENGENHEIRO(A)"><label for="cargo">Engenheiro(a)</label><br>
                    <input type="radio" id="cargo" name="cargo" style="text-transform: uppercase" value="ANALISTA">     <label for="cargo">Analista</label><br>
                    <input type="radio" id="cargo" name="cargo" style="text-transform: uppercase" value="ASSISTENTE">   <label for="cargo">Assistente</label><br>
                    <input type="radio" id="cargo" name="cargo" style="text-transform: uppercase" value="OPERADOR(A)">  <label for="cargo">Operador(a)</label><br>
                </div>
                <div class="boxBotoes">
                    <input type="button" id="" name="voltar" value="Voltar" onclick="location.href='P14.AtualizaFuncionario.php'">
                    <input type="button" id="" name="sair"   value="Sair"   onclick="location.href='P11.MenuAdmin.php'">
                </div>
                <div class="boxSub">
                    <form action="" method="POST">
